...
import { FaTrash, FaUpload, FaDownload, FaEye, FaEnvelope, FaPlus, FaFileAlt, FaPen, FaHistory } from "react-icons/fa";
import {
  DialogOverlay,
  DialogContainer,
  DialogHeader,
...
const FileItemContainer = styled.div<{ disabled: boolean; state: DocumentState; status?: string; hasVersions?: boolean }>`
  display: flex;
  align-items: center;
  ...
  background-color: ${p =>
    p.status === 'Signed' ? '#e3f6ec' :
    p.status === 'Filled' ? '#fff7e0' :
    p.state === DocumentState.MISSING ? '#fff8f8' :
    p.disabled ? '#f9f9f9' : '#ffffff'};
  border: 1px solid ${p => 
    p.status === 'Signed' ? '#c5e8d5' :
    p.status === 'Filled' ? '#ffe7b3' :
    p.state === DocumentState.MISSING ? '#ffdddd' : '#f0f0f0'};
  border-radius: 10px;
  ...
  &:hover {
    background-color: ${p =>
      p.status === 'Signed' ? '#d5f0e2' :
      p.status === 'Filled' ? '#fff0d0' :
      p.state === DocumentState.MISSING ? '#fff5f5' :
      p.disabled ? '#f9f9f9' : '#f9f9f9'};
...
const FileTypeBadge = styled.span<{ state: DocumentState; status?: string }>`
  background-color: ${p => 
    p.status === 'Signed' ? '#e3f6ec' :
    p.status === 'Filled' ? '#fff7e0' :
    p.state === DocumentState.MISSING ? '#ffefef' : '#f0f7ff'};
  color: ${p => 
    p.status === 'Signed' ? '#1d8450' :
    p.status === 'Filled' ? '#b0851f' :
    p.state === DocumentState.MISSING ? '#ff6b6b' : '#0071e3'};
  font-size: 9px;
  font-weight: 500;
...
const FileName = styled.div<{ state: DocumentState; status?: string }>`
  ...
  color: ${p => 
    p.status === 'Signed' ? '#1d8450' :
    p.status === 'Filled' ? '#b0851f' :
    p.state === DocumentState.MISSING ? '#ff6b6b' : '#333'};
  white-space: nowrap;
  overflow: hidden;
      ...
      <DialogOverlay onClick={() => setShowUploadStatusDialog(false)}>
        <DialogContainer onClick={e => e.stopPropagation()}>
          <DialogHeader>
            ...
            <DialogCloseButton onClick={() => setShowUploadStatusDialog(false)}>×</DialogCloseButton>
          </DialogHeader>
          <div style={{ padding: '20px' }}>
            <p>בחר את סטטוס המסמך שברצונך להעלות:</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '15px' }}>
              <Button 
                onClick={() => { setShowUploadStatusDialog(false); handleUpload('manual', DocumentState.UPLOADED); }}
                style={{ 
              ...
              >
                ריק 
                <span style={{ position: 'absolute', right: '10px', width: '12px', height: '12px', borderRadius: '50%', backgroundColor: '#0071e3' }}></span>
              </Button>
              <Button 
                onClick={() => { setShowUploadStatusDialog(false); handleUpload('manual', 'Filled'); }}
                style={{ 
              ...
              >
                מלא עם פרטים 
                <span style={{ position: 'absolute', right: '10px', width: '12px', height: '12px', borderRadius: '50%', backgroundColor: '#b0851f' }}></span>
              </Button>
              <Button 
                onClick={() => { setShowUploadStatusDialog(false); handleUpload('manual', DocumentState.SIGNED); }}
                style={{ 
              ...
              >
                חתום 
                <span style={{ position: 'absolute', right: '10px', width: '12px', height: '12px', borderRadius: '50%', backgroundColor: '#1d8450' }}></span>
              </Button>
            </div>
          </div>
        </DialogContainer>
  ...
  return (
    <>
      ...
      <FileInfo>
        ...
        <FileDetails>
          <FileTypeBadge state={state} status={status}>{fileType}</FileTypeBadge>
          <FileName 
            state={state} 
            status={status}
            ...
            <>
              ...
              {status !== DocumentState.SIGNED && status !== DocumentState.GENERAL && (
                <ActionButton
                  ...
                  style={{ 
                    color: status === DocumentState.FILLED ? '#b0851f' : '#0071e3'
                  }}
                >
                  <FaUpload />
                </ActionButton>
                      ...
                      <div key={version.id} style={{ 
                        ...
                        <div>
                          ...
                          <div style={{ 
                            fontSize: '12px', 
                            marginTop: '4px',
                            color: version.status === 'Signed' ? '#1d8450' : 
                                  version.status === 'Filled' ? '#b0851f' : '#0071e3',
                            fontWeight: 'bold'
                          }}>
                            {version.status}
                          </div>
...
const FileArea: React.FC<FileAreaProps> = ({
  ...
  const handleRequestUpload = (fileType: string, file: File, mode: 'auto' | 'manual' = 'manual', status?: string) => {
      if (onUpload) {
      ...
      <FileAreaContent>
        <FileListContainer>
          {files.length === 0 ? (
            ...
            Object.entries(groupedFiles).map(([fileType, typeFiles], groupIndex) => (
              <div key={`group-${fileType}`}>
                {/* Files of this type */}
                {typeFiles.map((file, index) => {
                  console.log(`Rendering FileItem for ${file.fileType} with versions:`, file.versions);
                  return (
              <FileItem
